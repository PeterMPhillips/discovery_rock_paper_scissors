...
module.exports = async function(deployer, network, accounts) {
  ...
  deployer.deploy(RockPaperScissors, enigma.enigmaContract._address, 1).then(function(rockPaperScissors){
    console.log(`Smart Contract "RockPaperScissors.Sol" has been deployed at ETH address: ${rockPaperScissors.address}`);
    const config = {
      filename: 'rock_paper_scissors.wasm',
      fn: 'construct(address)',
      args: [
        [rockPaperScissors.address, 'address']
      ],
      gasLimit: 2000000,
      gasPrice: utils.toGrains(1),
      from: accounts[0]
    };
    deploySecretContract(config).then(function(address){
